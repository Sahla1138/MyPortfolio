               ...
               try {   
                 body = await req.json();  } 
                     ...
                     return NextResponse.json({ error: 'Invalid JSON body' }, { status: 400 });  } 
                      const parsed = ContactSchema.safeParse(body);  if (!parsed.success)
                         {    
                       
                            return NextResponse.json(      { error: 
                            'Validation failed', issues: parsed.error.issues.map(i => i.message) },      { status: 400 }    );  } 
                             
                            const ip = req.headers.get('x-forwarded-for') || 'unknown';  
                             
                            const limited = rateLimitCheck(ip);  if (limited)
                                 {  
                                      return NextResponse.json({ error: 'Too many requests' }, { status: 429 });  }  
                            
                            const captchaOk = await verifyTurnstile();  if (!captchaOk) {  
                                  ...
                                  const payload = {   
                                     tenantId: env.TENANT_ID,  
                                      ...
                                      timestamp: new Date().toISOString()    }  }; 
                                       ...
                                       try {    const upstream = await fetch(`${env.RAISUITE_API_BASE}/enquiry`,
                                       {      method: 'POST',    
